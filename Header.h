...
public:
  ...
  Monom(const double _coefficient, const int _degrees) {
    if (_degrees > 999)
      throw std::string("degrees cant be more than 999");
    if (_degrees < 0)
      throw std::string("Degrees cannot be negative");
    if (std::isnan(_coefficient) || std::isinf(_coefficient))
      throw std::string("Degrees cannot be null or inf");
...
public:
  ...
  void ptr_add_new_monom(Monom *m) {
    if (m == nullptr)
      throw std::string("Cannot add null monom pointer");
    if (head == nullptr) {
      head = m;
      head->next = nullptr;
      count = 1;
  ...
  Polinom(const Polinom &other) : head(nullptr), count(0) {
    if (other.head) {
      ...
      while (currentOther) {
        Monom *newNode =
            new Monom(currentOther->coefficient, currentOther->degrees);
    ...
    while (current1 != nullptr && current2 != nullptr) {
      if (current1->degrees > current2->degrees) {
        ...
        if (std::abs(current1->coefficient + current2->coefficient) >= 1e-10) {
          Monom *to_add = new Monom(
              current1->coefficient + current2->coefficient, current1->degrees);
          ans.ptr_add_new_monom(to_add);
        }
        current1 = current1->next;
        current2 = current2->next;
  ...
  Polinom operator*(const Polinom &pol) {
    ...
    while (current1 != nullptr) {
      while (current2 != nullptr) {
        ...
        Monom *to_add = new Monom(current1->coefficient * current2->coefficient,
                                  current1->degrees + current2->degrees);
        ans.ptr_add_new_monom(to_add);
        current2 = current2->next;
      }
      current1 = current1->next;
